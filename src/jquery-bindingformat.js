...
(function($){
    ...
    var BindingFormat = function (type, element, options){
        ...
        this.regional = $.extend(true, this.regional, $.fn.bindingFormat.regional);
    ...
    BindingFormat.prototype = {
        constructor: BindingFormat,

        init: function(options){
            this.options = $.extend({}, FORMATS, options);
            this.hidden = this._appendHiddenInput();
        },
    ...
    $.fn.bindingFormat = function(type, options){
        if ($.inArray(type, ["float", "date"]) != -1) {
            return this.each(function(){
                var $this = $(this),
                    data = $this.data('bindingformat');

                if (!data) {
                    $this.data('bindingformat', new BindingFormat(type, this, options));
                    $this.change();
                }
            });
        } else {
            var data = $(this).data('bindingformat');
            if (data) {
                if (data[type]) {
                    return data[type].apply(data, Array.prototype.slice.call(arguments, 1));
                } else {
                    $.error('Method ' + type + ' does not exist on jQuery.bindingFormat');
                    return false;
                }
            }
        }
